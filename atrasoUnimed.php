...
$tabela = "SELECT registro, nome, date_format(data_envio_just_unimed, '%d-%m-%Y') as data_envio_just_unimed, date_format(ent_fat_just_dia, '%d-%m-%Y') as ent_fat_just_dia, just_atraso FROM controle WHERE ((data_envio_just_unimed <> '0000-00-00' AND ent_fat_just_dia IS NULL) OR (data_envio_just_unimed <> '0000-00-00' AND ent_fat_just_dia = '0000-00-00')) AND datediff(date(now()), data_envio_just_unimed) >= 10 AND particular IS NULL ORDER BY data_envio_just_unimed ASC";
$resultado = mysqli_query($conexao, $tabela);
?>
<?php
//TABELA REFERENTE A AVALIAÇÃO
$tabelaa = "SELECT registro, nome, date_format(data_envio_aval_unimed, '%d-%m-%Y') as data_envio_aval_unimed, date_format(ent_fat_aval_dia, '%d-%m-%Y') as ent_fat_aval_dia, just_atraso FROM controle WHERE ((data_envio_aval_unimed <> '0000-00-00' AND ent_fat_aval_dia IS NULL) OR (data_envio_aval_unimed <> '0000-00-00' AND ent_fat_aval_dia = '0000-00-00')) AND  datediff(date(now()), data_envio_aval_unimed) >= 10 AND particular IS NULL ORDER BY data_envio_aval_unimed ASC;";
 ...
 <!-- FIM TÓPICO -->
        ...
        <div class="intro">
			...
			<div class="col-md-13">
                            ...
                            <div class="alert alert-danger" role="alert">Não possui atrasos na Unimed sem justificativa!</div>
                            <?php } ?>
                                    ...
                                    <tbody>
                                        <?php while($linha = mysqli_fetch_assoc($resultado)){ ?>
                                            <tr>
                                                ...
                                                <?php if($linha['just_atraso'] == '' || $linha['just_atraso'] == null){  ?>
                                                <td><a href="justificarAtraso.php?registro=<?php echo $linha['registro']; ?> " class="btn btn-primary btn-sm">Justificar</a></td>
                                                <?php }else{ ?>
                                                <td><a href="justificarAtraso.php?registro=<?php echo $linha['registro']; ?> " class="btn btn-success btn-sm">Justificado</a></td>
                                                <?php } ?>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
                                    ...
                                    <tbody>
                                        <?php while($linhaa = mysqli_fetch_assoc($resultadoo)){ ?>
                                            <tr>
                                                ...
                                                <td><a href="justificarAtraso.php?registro=<?php echo $linhaa['registro']; ?> " class="btn btn-primary btn-sm">Justificar</a></td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>                                    
                                </table>
